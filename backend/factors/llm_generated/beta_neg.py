...
class BetaNeg(BaseFactor):
    factor_id = 'beta_neg'
    display_name = '市场贝塔（-rolling_beta, 60d）（取负）'
    category = 'volatility'
    description = '60 日滚动 CAPM beta（cs_mean 代理市场），取负后低 beta 看多。'
    hypothesis = '【已取负，方向反转】低 beta 股票风险调整后收益更高——低波动异象的贝塔维度（Frazzini-Pedersen 2014）。'
    params_schema: dict = {'window': {'type': 'int', 'default': 60, 'min': 20, 'max': 252, 'desc': '滚动回归窗口（交易日）'}}
