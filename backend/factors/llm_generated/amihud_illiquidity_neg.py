...
class AmihudIlliquidityNeg(BaseFactor):
    factor_id = 'amihud_illiquidity_neg'
    display_name = 'Amihud 非流动性（-|ret|/dollar_vol_mean）（取负）'
    category = 'volume'
    ...
    def compute(self, ctx: FactorContext, params: dict) -> pd.DataFrame:
        ...
        volume = ctx.data.load_panel(ctx.symbols, data_start, ctx.end_date.date(), freq='1d', field='volume', adjust='none')
        if close is None or close.empty or volume.empty:
            return -pd.DataFrame()
        ret = close.pct_change(fill_method=None).abs()
        dollar_vol = close * volume / 1000000.0
